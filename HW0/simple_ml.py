...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    Args:
        X(m * n) (np.ndarray[np.float32]): 2D input array of size
            (num_examples x input_dim). 
        y(m) (np.ndarray[np.uint8]): 1D class label array of size (num_examples,) 
        W1(n * d) (np.ndarray[np.float32]): 2D array of first layer weights, of shape
            (input_dim, hidden_dim)
        W2(d * k) (np.ndarray[np.float32]): 2D array of second layer weights, of shape

...
class Transpose(TensorOp):
    ...
    def compute(self, a):
        # BEGIN YOUR SOLUTION
        axes = self.axes
        axe0, axe1 = axes if axes else (a.ndim - 2, a.ndim - 1)
        permute_axes = list(range(a.ndim))
        permute_axes[axe0], permute_axes[axe1] = axe1, axe0
...
class Reshape(TensorOp):
    ...
    def gradient(self, out_grad, node):
        # BEGIN YOUR SOLUTION
        raise out_grad.reshape(node.inputs[0].shape)
        # END YOUR SOLUTION
...
class Tanh(TensorOp):
    ...
    def gradient(self, out_grad, node):
        # BEGIN YOUR SOLUTION
        return multiply(add_scalar(negate(power_scalar(tanh(node.inputs[0]), 2)), 1), out_grad)
        # END YOUR SOLUTION
